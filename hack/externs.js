...
function generateExterns(typeChecker, sourceFile, host, moduleResolutionHost, options) {
        ...
        for (const member of decl.members) {
            switch (member.kind) {
                ...
                case ts.SyntaxKind.SetAccessor:
                    // GetAccessor and SetAccessor deal by lear
                    let curmemberName = namespace;
                    if (member.name) {
                        curmemberName = curmemberName.concat([member.name.getText()]);
                    }
                    emit(`\n${typeName}.${curmemberName.join('.')} = {};\n`);
                    continue;
                default:
    ...
    function addImportAlias(node, moduleUri, name) {
        ...
        if (!moduleSymbol) {
            transformer_util_1.reportDiagnostic(diagnostics, moduleUri, `imported module has no symbol`);
            return;
        }
        const googNamespace = googmodule_1.namespaceForImportUrl(moduleUri, diagnostics, moduleUri.text, moduleSymbol);
        let aliasName;
        if (googNamespace) {
            ...
            const fullUri = googmodule_1.resolveModuleName(host, sourceFile.fileName, moduleUri.text);
